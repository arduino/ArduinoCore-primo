/*
  ...
  NFC library.
  It first initialize the NFC and then set a app message
  specifying the package name (for Android) and the ID
  application (for Windows phone), so that when a device
  with NFC is near to the board it will try to open the
  application (if present) or will look for the app in
  the store. Finally it register a callback function that
  will be called any time an NFC field is detected (it
  means that a device is near). 
