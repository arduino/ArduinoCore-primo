/*
  ...
  CoreSensors library.
  The example is to show how to detect the tilt event 
  with the Primo Core. Trying to tilt the Primo Core, 
  when the event is triggered, the USER LED will blink once.
...
void setup() {
  ...
  coresensors.enableTiltDetection();
  // set up interrupt pin to sensor
  attachInterrupt(INT1, Tilt, RISING);
}
...
void loop() {
  ...
  if (tilt) {
    ...
    if (status)
    {
      ...
      bleSerial.println("Tilt Detected!");
    }
  }
}
