...
#define DEVICE_NAME                      "Arduino_Primo"                               /**< Name of device. Will be included in the advertising data. */
#define MANUFACTURER_NAME                "Arduino org"                      /**< Manufacturer. Will be passed to Device Information Service. */
...
static const char url[] =
    {'a', 'r', 'd', 'u', 'i', 'n', 'o', '.', 'o', 'r', 'g'}; //URL "nordicsemi.com"
		
uint8_t ndef_msg_buf_a[256];
...
/**@brief Function for nfc url record.
 *
 * @details This function does not stop and start the NFC tag data emulation.
 *
 */
static void nfc_url_record_set(void);
...
static void nfc_url_record_set(void)
{
    ...
    /* Provide information about available buffer size to encoding function */
    uint32_t len = sizeof(ndef_msg_buf_a);
    ...
    ret_val = nfcSetPayload( (char*)ndef_msg_buf_a, len);
    if (ret_val != NFC_RETVAL_OK)
    {
        APP_ERROR_CHECK((uint32_t) ret_val);
    }
